...
class Upsample(keras.Model):
    def __init__(self, hidden):
        ...
        self.conv_hr1 = keras.layers.Conv2D(
            hidden, 3, padding="same", activation=keras.layers.ReLU(negative_slope=0.1))
        self.conv_hr2 = keras.layers.Conv2D(
            hidden, 3, padding="same", activation=keras.layers.ReLU(negative_slope=0.1))
...
class IMDModule(keras.Model):
    def __init__(self, in_channels, c1=20, c2=12, c3=4, distillation_rate=0.5):
        ...
        self.c1 = keras.layers.Conv2D(
            c1, 3, padding="same", activation=keras.layers.ReLU(negative_slope=0.1))
        self.c2 = keras.layers.Conv2D(
            c2, 3, padding="same", activation=keras.layers.ReLU(negative_slope=0.1))
        self.c3 = keras.layers.Conv2D(
            c3, 3, padding="same", activation=keras.layers.ReLU(negative_slope=0.1))
        self.c5 = keras.layers.Conv2D(in_channels, 1, padding="same")
    ...
    def call(self, x):
        ...
        distilled_c2 = out_c2[:, :, :, 0:self.distilled_channels]
...
class BidirectionalRestorer_V6(keras.Model):
    def __init__(self, hidden_channels=8):
        ...
        self.shirking1 = keras.layers.Conv2D(
            2*hidden_channels, 1, 1, padding="same")
    ...
    def call(self, inputs):
        ...
        now_frame = self.concat_b([now_frame_forward, now_frame_backward])
        hidden = now_frame
        res.append(hidden)
        for i in range(1, T):
        ...
        gap = 1
        for i in range(0, T, gap):
            t = []
            for j in range(gap):
                t.append(self.concat_c(
                    [res[i + j][0:1, :, :, :], res[T-i-j-1][1:2, :, :, :]]))
            t_0_1_res = self.upsample([self.concat_b(t), h, w])
            for j in range(gap):
                res2.append(t_0_1_res[j:j+1, :, :, :])
        res = self.concat_c(res2)
        return res + biup

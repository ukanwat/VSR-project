...
class BaseMatchDataset(BaseDataset):
    ...
    def __init__(self, pipeline, mode="train"):
        ...
        self.pooling = defaultdict(list)
        self.difficulty_score = {1: 0.4, 2: 0.4, 3: 0.4,
    ...
    def evaluate(self, results, save_path):
        ...
        for res in results:
            ...
            for metric, val in res.items():
                ...
                if val > 5:
                    ...
                    eval_results[metric + "_" +
                                 str(class_id) + "_more_than_5_nums"].append(1.0)
                else:

                    integer = str(int(val))
                    point = str(int(val*10) % 10)
                    eval_results[metric + "_" +
                                 "{}.{}".format(integer, point) + "_nums"].append(1.0)
        ...
        for class_id, diff_score in self.difficulty_score.items():
            ...
            if eval_results.get(key) is None:
                self.logger.info(
                    "do not have class index: {}".format(class_id))
            else:
                thre = self.threshold[class_id]
                list_0_1 = [(dis_value <= thre)
                            for dis_value in eval_results[key]]
                Lambda = sum(list_0_1) * 1.0 / len(list_0_1)
                for dis_value in eval_results[key]:
                    now_score += get_score_by_dis(dis_value) * \
                        diff_score * Lambda

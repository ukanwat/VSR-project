...
class MinimumBoundingBox_ByOpticalFlow(object):
    ...
    def __call__(self, results):
        ...
        segments_lq_first_frame = slic(
            results['lq'][0], n_segments=n_segments, compactness=compactness, start_label=0)

        max_class_id = np.max(segments_lq_first_frame)
        select_class_id = random.randint(0, max_class_id)
        lq_masks = []
        mask_lq_first_frame = np.argwhere(
            select_class_id == segments_lq_first_frame)
        ...
        for idx in range(first_frame_idx, first_frame_idx + len(lq_paths) - 1):
            ...
            flowpath = os.path.join(self.flow_dir, clipname, "{}_{}.npy".format(
                str(idx).zfill(self.name_padding_len), str(idx+1).zfill(self.name_padding_len)))
        ...
        for idx in range(0, len(lq_masks)):
            tl = np.min(lq_masks[idx], axis=0)
            br = np.max(lq_masks[idx], axis=0)

            tl = (np.floor(tl / self.blocksizes) *
                  self.blocksizes).astype(np.int64)
            br = (np.ceil((br+1) / self.blocksizes) *
                  self.blocksizes).astype(np.int64) - 1

            length_h = br[0] - tl[0] + 1
            length_w = br[1] - tl[1] + 1
            lq_crops.append(
                results['lq'][idx][tl[0]:tl[0] + length_h, tl[1]:tl[1] + length_w, ...])
            gt_crops.append(results['gt'][idx][self.scale * tl[0]:self.scale * (
                tl[0] + length_h), self.scale*tl[1]: self.scale*(tl[1] + length_w), ...])
...
class Random_Crop_Opt_Sar(object):
    ...
    def get_optical_h_w(self, sar_h, sar_w):
        up = 800 - self.size[0]
        optical_h = random.randint(
            max(sar_h - (self.size[0]-self.size[1]), 0), min(sar_h, up))
        optical_w = random.randint(
            max(sar_w - (self.size[0]-self.size[1]), 0), min(sar_w, up))
    ...
    def __call__(self, results):
        ...
        results['sar'] = results['sar'][sar_h:sar_h +
                                        self.size[1], sar_w:sar_w+self.size[1], :]
        ...
        if self.Contrast:
            ...
            for _ in range(3):
                ...
                results['opt'].append(
                    opt[optical_h:optical_h+self.size[0], optical_w:optical_w+self.size[0], :])
            ...
            optical_h, optical_w = self.get_optical_h_w(sar_h, sar_w)
            results['opt'] = results['opt'][optical_h:optical_h +
                                            self.size[0], optical_w:optical_w+self.size[0], :]
...
class PairedRandomCrop(object):
    ...
    def __call__(self, results):
        ...
        if h_gt != h_lq * scale or w_gt != w_lq * scale:
            raise RuntimeError(
                "HR's size is not {}X times to LR's size".format(scale))

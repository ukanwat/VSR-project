...
class BaseSRDataset(BaseDataset):
    ...
    def scan_folder(path):
        ...
        images = sorted(
            list(scandir(path, suffix=IMG_EXTENSIONS, recursive=True)))
    ...
    def evaluate(self, results):
        ...
        """
        assert is_list_of(
            results, dict), f'results must be a list of dict, but got {type(results)}'
        assert len(results) >= len(
            self), "results length should >= dataset length, due to multicard eval"
        self.logger.info("eval samples length: {}, dataset length: {}, only select front {} results".format(
            len(results), len(self), len(self)))
...
class BaseVSRDataset(BaseDataset):
    ...
    def test_aggre(self, save_path, padding_len=4, start_index=1):
        clip_names = sorted(self.frame_num.keys())
        ...
        for _ in range(len(self)):
            ...
            if do_frames == frame_nums[now_clip_idx]:
                ...
                for idx in range(total_deal, total_deal + do_frames):
                    ...
                    desti_path = osp.join(save_dir_path, str(
                        idx - total_deal + start_index).zfill(padding_len) + ".png")
                    if osp.exists(ensemble_path_1):
                        ...
                        for e in range(1, 8):
                            path = osp.join(
                                save_path, "idx_{}_epoch_{}.png".format(idx, e))
                            sum_result = sum_result + \
                                imread(path, flag='unchanged').astype(
                                    np.float32)
                            os.remove(path)
                        ...
                        shutil.move(osp.join(save_path, "idx_" +
                                    str(idx) + ".png"), desti_path)
    ...
    def evaluate(self, results, save_path):
        ...
        for res in results:
            ...
            if do_frames == frame_nums[now_clip_idx]:
                clip_name = clip_names[now_clip_idx]
                self.logger.info("{}: {} is ok".format(
                    now_clip_idx, clip_name))
                for metric, values in eval_results_one_clip.items():
                    ...
                    plt.plot(list(range(len(values))), values, label=metric)
                    plt.title(title)
                    plt.xlabel('frame idx')
                    plt.ylabel('{} value'.format(metric))
                    plt.legend()
                    fig = plt.gcf()
                    fig.savefig(osp.join(save_SVG_path, save_filename +

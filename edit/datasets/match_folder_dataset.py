...
class MatchFolderDataset(BaseMatchDataset):
    ...
    def load_annotations(self):
        ...
        ban_dict = [(5, 95), (5, 19), (5, 20), (5, 10), (5, 44), (5, 9), (3, 193), (3, 61), (1, 187), (1, 185), (1, 179), (1, 30),
                    (1, 95), (1, 59), (1, 52), (1, 94), (1, 26), (1, 24), (1, 23), (1, 43), (1, 3), (1, 82), (2, 455), (6, 375), (6, 329)]
        ...
        for i in range(len(opt_list)):
            if self.mode == "train":
                ...
                raise NotImplementedError(
                    "not known mode: {}".format(self.mode))
        ...
        if self.mode == "train":
            ...
            if self.balance_flag == "test":
                test_distribute = [400, 400, 400, 600, 300, 600]
                data_infos = []
                for i in range(len(test_distribute)):
                    data_infos = data_infos + \
                        random.sample(ans[i+1], test_distribute[i])
                assert len(data_infos) == sum(test_distribute)

            elif self.balance_flag == "uniform":
                uniform_distribute = [313, 313, 313, 314, 313, 314]
                data_infos = []
                for i in range(len(uniform_distribute)):
                    data_infos = data_infos + \
                        random.sample(ans[i+1], uniform_distribute[i])
                assert len(data_infos) == sum(uniform_distribute)
            else:
                pass

        self.logger.info("MatchFolder dataset load ok, mode:{} len:{}".format(
            self.mode, len(data_infos)))
        return data_infos

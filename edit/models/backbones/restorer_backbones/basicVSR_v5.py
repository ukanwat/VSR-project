...
class IMDModule(M.Module):
    def __init__(self, in_channels, c1=20, c2=12, c3=4, distillation_rate=0.5):
        ...
        self.c2 = M.Conv2d(c1 - self.distilled_channels,
                           c2, 3, stride=1, padding=1)
        self.c3 = M.Conv2d(c2 - self.distilled_channels,
                           c3, 3, stride=1, padding=1)
        self.act = M.LeakyReLU(negative_slope=0.1)
        self.c5 = M.Conv2d(2*self.distilled_channels + c3 +
...
class Upsample(M.Module):
    def __init__(self, hidden):
        ...
        self.shirking = M.Conv2d(
            4*hidden, hidden, kernel_size=1, stride=1, padding=0)
        self.sel = SEL(hidden)
        self.reconstruction = IMDModule(in_channels=hidden)
        self.conv_last = M.Conv2d(
            hidden, 3, kernel_size=3, stride=1, padding=1)
        self.conv_hr1 = M.Conv2d(
            hidden, hidden, kernel_size=3, stride=1, padding=1)
        self.conv_hr2 = M.Conv2d(
            hidden, hidden, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, inputs):
        ...
        out = F.nn.interpolate(inp=out, scale_factor=2,
...
class BasicVSR_v5(M.Module):
    def __init__(self, in_channels,
        ...
        self.conv1 = M.Conv2d(in_channels, hidden_channels,
        ...
        self.shirking1 = M.Conv2d(
            4*hidden_channels, 2*hidden_channels, kernel_size=1, stride=1, padding=0)

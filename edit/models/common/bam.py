...
class ChannelGate(M.Module):
    ...
    def forward(self, in_tensor):
        avg_pool = F.avg_pool2d(in_tensor, (in_tensor.shape[2], in_tensor.shape[3]), stride=(
            in_tensor.shape[2], in_tensor.shape[3]))
        x = self.gate_c(avg_pool)
        x = F.expand_dims(x, axis=[2, 3])
...
class SpatialGate(M.Module):
    def __init__(self, gate_channel, reduction_ratio=16, dilation_conv_num=2, dilation_val=4):
        ...
        self.gate_s = M.Sequential(
            M.Conv2d(gate_channel, gate_channel //
                     reduction_ratio, kernel_size=1),
